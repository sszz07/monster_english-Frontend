...
export default function AdventureContainer({ placeData: initialPlaceData }: AdventureContainerProps) {
  ...
  //==================== 진행률 계산 ====================
  const totalWords = masterRegions.length; // 영상 갯수
  const clickedCount = clickedSet.size; // 클릭된 단어 갯수  그래서 totalWords와clickedCount 카운트가 같다면 진행률을 계산을 할수 있음
  ...
  const SentenceAudios = (audioUrl?: string) => {
    stopCurrentAudio();
    if (!audioUrl) {
      console.warn("오디오 파일이 지정되지 않았습니다.");
      return;
    }
    ...
    audio.play().catch((err) => {
      console.error("문장 오디오 재생 에러:", err);
    });
  ...
  // 단어 mp3 재성
  const WordAudios = (targetId: string) => {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-start pb-16 w-full relative"
      ...
      <div className="flex gap-4 bg-white/40 backdrop-blur-md p-1.5 rounded-full shadow-inner border border-white/40 mb-8">
        <button
          onClick={() => { stopCurrentAudio(); setCurrentMode("explore"); }}
          className={`flex items-center gap-2 px-6 py-2.5 rounded-full font-extrabold text-base transition-all duration-300 ${currentMode === "explore" ? "bg-[#4CAF50] text-white shadow-md scale-105" : "text-emerald-800 hover:bg-white/30"
            }`}
        >
          🔍 Explorer
        </button>
        ...
        >
          {isGameUnlocked ? "🎮 Game Mode" : "🔒 Game Mode"}
        </button>
      </div>
              ...
              {masterRegions.map((region) => {
                ...
                return (
                  <polygon
                    ...
                    onClick={() => WordAudios(region.wordKey)}
          ...
          <div className="flex justify-between items-center pt-2 px-2 gap-4">
            ...
            <div className="flex gap-3">
              <button
                ...
                className={`font-black text-sm px-5 py-3 rounded-xl shadow-sm ${hintOn ? "bg-amber-500 text-white" : "bg-white text-amber-600 border border-amber-200"
                  }`}
              >
                {hintOn ? "💡 Hint On" : "✨ Hint"}
              </button>
              ...
              >
                {isGameUnlocked ? "🎮 Game Ready" : `🔒 Lock`}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-[#FFFBF0] rounded-[40px] border-8 border-white p-6 max-w-3xl w-full flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-2 bg-white/60 border p-4 rounded-xl shadow-inner">
              {(() => {
                // 1번째, 2번째 문장 오디오 URL을 순서대로 배열로 생성
                const audioUrls = [
                  videoTarget.sentenceFirstAudioUrl,
                  videoTarget.sentenceSecondAudioUrl,
                ];

                return videoTarget.sentence
                  .split('.')
                  .map((s) => s.trim())
                  .filter(Boolean)
                  .map((sentenceStr, idx) => (
                    <div key={idx} className="flex justify-between items-center p-3 bg-white border rounded-xl shadow-sm">
                      ...
                      <button
                        onClick={() => SentenceAudios(audioUrls[idx])}
                        className="bg-[#4CAF50] hover:bg-[#43A047] text-white px-3 py-1.5 rounded-xl font-bold text-xs shadow-[0_3px_0_#2E7D32] transition-colors"
                      >
                        🔊 Listen
                      </button>
                    </div>

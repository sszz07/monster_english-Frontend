...
interface PictureQuizModalProps {
  currentQuestion: {
    ...
    imageType?: string; // 👈 "apartment", "house", "school" 등 다양한 장소 문자열 수용
  };
  ...
  onWrongAnswer: () => void;
  placeImages?: Record<string, string>; // 👈 GameContainer에서 넘어오는 이미지 맵
  defaultImg?: string;                  // 👈 기본 배경 이미지
}
...
export default function PictureQuizModal({
  ...
  onCorrectAnswer,
  onWrongAnswer,
  placeImages = {}, // 기본값 빈 객체
  defaultImg = ""   // 기본값 빈 문자열
  ...
  // 🌟 질문의 imageType 키로 이미지를 찾고, 없으면 defaultImg를 사용합니다!
  const activeImage = (currentQuestion.imageType && placeImages[currentQuestion.imageType]) || defaultImg;
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center z-50 p-4">
      <div className="bg-[#FFFDF6] rounded-[2rem] border-4 border-white shadow-2xl max-w-3xl w-full p-6 flex flex-col gap-4 relative">
        <div className="flex justify-between items-start">
          ...
          <button onClick={closeGameModal} className="text-gray-400 hover:text-gray-600 bg-gray-100 p-1.5 rounded-full">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="w-full h-48 bg-slate-800 rounded-2xl overflow-hidden relative flex items-center justify-center shadow-inner border border-gray-100">
          ...
          >
            {/* 👈 동적으로 매핑된 activeImage 랜더링 */}
            <img src={activeImage} alt="Quiz Piece" className="w-full h-full object-contain" />
          </div>
          {feedback === "correct" && (
            <div className="absolute inset-0 bg-emerald-500/20 backdrop-blur-[1px] flex items-center justify-center text-white font-black text-xl">
              Correct! ✨
            </div>
          )}
          {feedback === "wrong" && (
            <div className="absolute inset-0 bg-rose-500/20 backdrop-blur-[1px] flex items-center justify-center text-white font-black text-xl">
              Try Again! ❌
            </div>
          )}
        </div>
          ...
          {choices.map((word, index) => {
            ...
            return (
              <button
                ...
                disabled={!!feedback}
                className={`py-4 rounded-xl font-black text-xl capitalize transition-all active:scale-[0.99] border ${style}`}
              >
                {word.replace(/_/g, " ")}
              </button>
            );

...
const BoardPage = () => {
    ...
    return (
        <div className="w-full">
            <div className="p-5 mx-auto bg-white min-h-screen max-w-[1300px] min-w-[900px]">
                <div className="flex justify-between">
                    <h1 className="text-3xl md:text-4xl font-extrabold text-[#2C3E50] mb-4 ml-4">Announcements</h1>
                ...
                <div className="flex bg-gray-50 border-b-2 border-gray-200 py-3 px-4 mt-4 font-bold text-gray-700 text-center">
                    ...
                    <div className="w-40">Write date</div>
                ...
                <div className="border border-gray-200 rounded-b-md">
                    {isLoading ? (
                        ...
                        posts.map((item) => (
                            ...
                            >
                                <div className="w-24">{item.announcementId}</div>
                                <div className="flex-1 text-left px-4 truncate font-medium text-gray-800">{item.title}</div>
                                <div className="w-40 text-sm text-gray-500">{new Date(item.createdAt).toLocaleDateString()}</div>
                                <div className="w-24">{item.viewCount}</div>
                            </div>
                        ))
                    )}
                </div>
